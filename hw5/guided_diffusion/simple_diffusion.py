...
class DDIMDiffusion(DDPMDiffusion):
    ...
    def p_sample(self, model, x, t, eta=0.0):
        ...
        variance, log_variance = self.var_processor.get_variance(var_values, t)   # get the log  of variance
...
class Repaint(DDIMDiffusion):
    ...
    def p_sample(
        ...
        ##############################################
        #####   TODO    #####
        ##### Here updated sample x_t refers to the noisy image, where the known region is          #####
        ##### obtained by adding noise to GT, and the unknown region is obtained from 'x', and      #####
        ##### the known and unknown region are combined using the ground truth mask (gt_keep_mask). #####
        ##### Compelete the implementation to compute the updated sample (x_t) for the              #####
        ...
        ##############################################
        #####   TODO    #####
        ##### One-step denoising using the model: Perform a forward pass on the model using the computed x_t.   #####
        ...
        #####  Start TODO   #####
        model_output = None
        pred_score, var_values = None
        #####  End TODO     #####
        
        
        model_mean, pred_xstart = self.mean_processor.get_mean_and_xstart(x_t, t, pred_score)
        variance, log_variance = self.var_processor.get_variance(var_values, t)   # get the log  of variance
