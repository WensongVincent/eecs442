...
import functools
from guided_diffusion.simple_diffusion import DDPMDiffusion
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        out = self.out(h)
        # coeff = extract_and_expand(self.alphas_cumprod, timesteps, out[:, :3])
        # coeff = extract_and_expand(self.alphas_cumprod, timesteps, out)
        # print('coeff')
        # print(out.shape)
        # out[:, :3] = -th.sqrt(1 - coeff) * out[:, :3]
        # out = -th.sqrt(1 - coeff) * out
        return out
...
def extract_and_expand(array, time, target):
    if type(array) is th.Tensor:
      array = array.numpy()
    array = th.from_numpy(array).to(target.device)[time].float()
